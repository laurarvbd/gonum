...
// makeNaNOf returns a new randomly filled m×n matrix of the underlying matrix type.
...
func testOneInput(t *testing.T,
	...
	for _, aMat := range testMatrices {
		for _, test := range sizes {
			...
			rr, rc := empty.Dims()
			neverEmpty := makeRandOf(receiver, rr, rc)
